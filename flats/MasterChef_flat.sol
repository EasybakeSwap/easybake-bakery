...
contract MasterChef is Ownable {
    ...
    function pendingOven(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 ovenReward = multiplier * (ovenPerSecond/chains) * pool.allocPoint / (totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 ovenReward = 
            bonusMultiplier * (ovenPerSecond/chains) * pool.allocPoint / totalAllocPoint;
