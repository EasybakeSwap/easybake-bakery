...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // The pending OVEN entitled to a user is referred to as the pending reward:
        //
    ...
    constructor(IERC20 _oven, address _treasury, uint256 _OVENPerBlock, uint256 _startBlock, uint256 _bonusEndBlock) public {
        oven = _oven;
        treasury = _treasury;
        devaddr = msg.sender;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 OVENReward = multiplier.mul(OVENPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        safeOVENTransfer(address(this), OVENReward);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) { // send pending OVEN rewards
            safeOVENTransfer(msg.sender, pending);
        }
    ...
    // SAFE TRANSFER FUNCTION | ACCOUNTS FOR ROUNDING ERRORS | ENSURES SUFFICIENT OVEN IN POOLS.
